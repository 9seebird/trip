...
export function PhotoReviews({project:p,onAction,selectedIndex,onSelect,onlyPending,onFilter}:Props&{selectedIndex:number|null;onSelect:(index:number)=>void;onlyPending:boolean;onFilter:(value:boolean)=>void}){
 ...
 return <section className="quality-panel"><h3>사진별 수정·재검수 <span className="demo-pill">{pending.length}장 재검수 필요</span></h3><p>수정 요청을 저장해도 현재 숙소 화면은 유지되며, 작업 상태만 보정 중으로 변경됩니다. 수정 선택을 누르면 아래에서 원본과 조절 미리보기를 확인하실 수 있습니다. 다시 누르면 선택이 해제됩니다. 재검수 완료한 메모는 사진별 수정 이력에 보관됩니다.</p><div className="review-context"><div><span>현재 작업 중인 숙소</span><strong>{p.name}</strong></div><span className={`status s${p.stage}`}>{stages[p.stage]}</span></div><label className="pending-toggle"><Checkbox checked={onlyPending} onCheckedChange={v=>onFilter(v===true)}/>재검수 필요한 사진만</label><div className="photo-review-list">{photos.map((photo,index)=>({photo,index})).filter(({index})=>!onlyPending||pending.some(r=>r.index===index)).map(({photo,index})=><PhotoReview key={index+':'+photo.name+':'+String(p.photoReviews?.find(r=>r.index===index)?.resolved)} photo={photo} index={index} review={p.photoReviews?.find(r=>r.index===index)} selected={selectedIndex===index} onSelect={()=>onSelect(index)} onAction={onAction}/>)}</div>{onlyPending&&!pending.length&&<p>재검수할 사진이 없습니다.</p>}<details className="photo-review-history"><summary>사진별 수정 이력 · {(p.photoReviews??[]).reduce((sum,review)=>sum+reviewEvents(review).length,0)}건</summary><p>요청과 재검수 완료 기록입니다. 위 사진 필터와 관계없이 확인하실 수 있습니다.</p>{(p.photoReviews??[]).length?(p.photoReviews??[]).map(review=><section key={review.index}><h4>{review.index+1}. {photos[review.index]?.category??'미분류'} · {photos[review.index]?.name}</h4>{reviewEvents(review).map((event,i)=><article key={i}><div><strong>{event.resolved?'재검수 완료':'수정 요청'}</strong><time>{event.at?new Date(event.at).toLocaleString('ko-KR'):'이전 기록 · 시간 정보 없음'}</time></div><p>{event.note}</p></article>)}</section>):<p>아직 저장된 수정 이력이 없습니다.</p>}</details></section>
}
function PhotoReview({photo,index,review,onAction,selected,onSelect}:{photo:{name:string;url:string;category?:string};index:number;review?:PhotoReviewRecord;onAction:Props['onAction'];selected:boolean;onSelect:()=>void}){
 ...
 return <article className={`photo-review ${selected?'comparison-selected':''}`}><button type="button" className="review-photo-select" aria-pressed={selected} onClick={onSelect}><img src={photo.url} alt={photo.name}/><span>{selected?'수정 중':'수정 선택'}</span></button><div><strong>{photo.category??'미분류'} · {photo.name}</strong><p>{review?(review.resolved?'재검수 완료':'재검수 필요'):'수정 요청 없음'}</p><label>사진 수정 메모<textarea maxLength={500} value={note} onChange={e=>setNote(e.target.value)} placeholder={review?.resolved?'새 수정 요청이 있으면 입력해 주세요.':'예: 창가 노출을 낮춰 주세요.'}/></label><div className="quality-buttons"><button className="secondary-button" disabled={!note.trim()} onClick={()=>onAction({type:'photo-review',index,note,resolved:false})}>수정 요청 저장</button>{review&&!review.resolved&&<button className="primary-button" disabled={note.trim()!==review.note} onClick={()=>{if(onAction({type:'photo-review',index,note:review.note,resolved:true}))setNote('')}}>확인 · 재검수 완료</button>}</div></div></article>
}
